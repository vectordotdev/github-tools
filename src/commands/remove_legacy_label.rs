    ...
    item["labels"]
        ...
        .map(|labels| {
            labels.iter().any(|l| {
                ...
                if case_insensitive {
                    lname.to_lowercase() == name.to_lowercase()
                } else {
                    lname == name
                }
            })
    ...
    for attempt in 0..5u32 {
        ...
        if status == 200 || status == 201 {
            ...
            if actual == type_name {
                ...
                return (
                    false,
                    "Type field not supported (issue may not be in project)".into(),
                );
            } else {
                return (
                    false,
                    format!("Type set to '{actual}' instead of '{type_name}'"),
                );
            }
        }
...
fn remove_label(
    ...
    if dry_run {
        ...
        return (
            true,
            format!("[dry-run] would DELETE /repos/{repo}/issues/{number}/labels/{encoded}"),
        );
    }
    let encoded = urlencoding::encode(label);
    ...
    for attempt in 0..5u32 {
        ...
        if status == 204 || status == 200 {
            return (true, format!("Removed label '{label}' from #{number}"));
        }
        if status == 404 {
            return (
                false,
                format!("Label '{label}' not present on #{number} (404)"),
            );
        }
        if status == 403 || (500..600).contains(&status) {
            backoff_sleep(attempt + 1);
        ...
        return (
            false,
            format!("Failed to remove label on #{number}: {status} {body}"),
        );
    }
    (
        false,
        format!("Failed to remove label on #{number} after retries"),
    )
}
...
pub fn run(args: &Args) -> Result<()> {
    ...
    println!(
        "Migrating '{}' -> Type field '{}'",
        args.legacy_label, type_info.name
    );
    ...
    if !args.dry_run
        && !crate::confirm(
            &format!(
                "This will modify labels on issues/PRs in: {}. Proceed?",
                args.repos.join(", ")
            ),
            args.yes,
        )
    ...
    'repos: for repo in &args.repos {
        ...
        for item in &items {
            ...
            if let Some(since) = &since_date {
                ...
                let created =
                    chrono::NaiveDateTime::parse_from_str(created_str, "%Y-%m-%dT%H:%M:%SZ")
                        .map(|dt| dt.date())
                        .unwrap_or(*since);
                if created < *since {
                    total_skipped += 1;
            ...
            if args.require_type_field && !has_correct_type {
                total_skipped += 1;
                println!(
                    "#{number}  SKIP (type field not set to '{}')  {title}",
                    type_info.name
                );
                continue;
            }
            ...
            if args.set_type_field && !has_correct_type && (!has_different_type || args.overwrite_type) {
                ...
                if !ok && !args.dry_run {
                    total_skipped += 1;
                    println!(
                        "#{number}  ERROR: Cannot set type field. Keeping legacy label and stopping."
                    );
